...
{
    public partial class ROIListItem : UserControl
    {
        ...
        public bool is_visible
        {
            ...
            set
            {
                ...
                if (is_visible)
                {
                    roi_toggle_icon.BackgroundImage = Resources.icon_toggle_act;
                }
                else
                {
                    roi_toggle_icon.BackgroundImage = Resources.icon_toggle;
                }
            }
        }
        ...
        public ROIListItem(bool is_visible, bool is_checked, string name, Color color)
        {
            InitializeComponent();
            InitializeItem(is_visible, is_checked, name, color);
            roi_name.DoubleClick += roi_name_DoubleClick; // 더블 클릭 이벤트 핸들러 추가
            roi_name.LostFocus += roi_name_LostFocus; // 포커스 잃었을 때 이벤트 핸들러 추가
        }

        private void InitializeItem(bool is_visible, bool is_checked, string name, Color color)
        {
            this.color = color;
            this.name = name;
        ...
        private void roi_toggle_icon_Click(object sender, EventArgs e)
        {
        ...
        {
            public ROIListItem ClickedItem { get; }

            public ItemClickedEventArgs(ROIListItem item)
            {
                ClickedItem = item;
            }
        }
